...
//	}
	...
	public ExApi updateData(Long id, ExApi exapi) {
		String link = url + "/" + id;
		ResponseEntity<ExApi> responseEntity = restTemplate.exchange(link, HttpMethod.PUT, new HttpEntity<>(exapi),
				ExApi.class);
	...
	public ExApi deleteData(Long id) {
		String link = url + "/" + id;
		ResponseEntity<ExApi> responseEntity = restTemplate.exchange(link, HttpMethod.DELETE, HttpEntity.EMPTY,
				ExApi.class);
